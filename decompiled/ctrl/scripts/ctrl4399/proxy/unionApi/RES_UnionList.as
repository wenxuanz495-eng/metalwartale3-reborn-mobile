package ctrl4399.proxy.unionApi
{
   ...
   public class RES_UnionList implements TBase
   {
      
      private static const STRUCT_DESC:TStruct = new TStruct("RES_UnionList");
      
      private static const TAG_FIELD_DESC:TField = new TField("tag",TType.STRING,1);
      
      private static const UNION_LIST_FIELD_DESC:TField = new TField("unionList",TType.LIST,2);
      
      private static const COUNT_FIELD_DESC:TField = new TField("count",TType.STRING,3);
      
      public static const TAG:int = 1;
      
      public static const UNIONLIST:int = 2;
      ...
      metaDataMap[UNIONLIST] = new FieldMetaData("unionList",TFieldRequirementType.DEFAULT,new ListMetaData(TType.LIST,new StructMetaData(TType.STRUCT,UnionList)));
      metaDataMap[COUNT] = new FieldMetaData("count",TFieldRequirementType.DEFAULT,new FieldValueMetaData(TType.STRING));
      FieldMetaData.addStructMetaDataMap(RES_UnionList,metaDataMap);
      ...
      public function set unionList(param1:Array) : void
      {
         this._unionList = param1;
      }
      
      public function unsetUnionList() : void
      {
         this.unionList = null;
      }
      
      public function isSetUnionList() : Boolean
      {
            ...
            switch(_loc2_.id)
            {
               ...
               case UNIONLIST:
                  if(_loc2_.type == TType.LIST)
                  {
                     ...
                     this.unionList = new Array();
                     _loc4_ = 0;
                     while(_loc4_ < _loc3_.size)
                     {
                        _loc5_ = new UnionList();
                        _loc5_.read(param1);
                        this.unionList.push(_loc5_);
                        _loc4_++;
                     }
                     param1.readListEnd();
                  }
      ...
      public function write(param1:TProtocol) : void
      {
         ...
         {
            param1.writeFieldBegin(UNION_LIST_FIELD_DESC);
            param1.writeListBegin(new TList(TType.STRUCT,this.unionList.length));
            for each(_loc2_ in this.unionList)
            {
               _loc2_.write(param1);
            }
            param1.writeListEnd();
      ...
      {
         var _loc1_:String = new String("RES_UnionList(");
