# -*- coding: utf-8 -*-
# 星际雷神 V26 修订（2026-09-18）：lv1（MK1）d10 蓄力特效链下移 1px——
# 四槽（f2/f4/f6/f9 @d10，characterId 2907/2909/2911/2913）matrix
# translateY 179→199（+20tw=+1px），translateX=-210（V25 值）不变，scale 0.75 不变；
...
SRC_XML  = "work"+BS+"sub1130-v26-src.xml"
OUT_XML  = "work"+BS+"sub1130-v26.xml"
OUT_SWF  = "work"+BS+"sub1130-v26.swf"
RB_XML   = "work"+BS+"sub1130-v26-readback.xml"
...
# 1) 底稿哈希断言：当前 swf == manifest == V25 哈希
...
assert actual == manifest_line.upper() and actual.startswith("78EAD0A2"), actual[:8]
...
assert_state(tree, "改前", OLD_TY)

# 3) 手术：仅四槽 translateY 下移 20tw
n = 0
for it in subtags(sprite(tree, '121')):
    if (it.get('type') or '') == 'PlaceObject2Tag' and it.get('depth') == '10' and it.get('characterId') in FX_IDS:
        ...
        n += 1
assert n == 4
assert_state(tree, "改后", NEW_TY)
print("lv1: 四槽特效 translateY 179→199（下移 1px）完成")
...
for i in leaf:
    assert 'translateX="' + TX + '" translateY="' + OLD_TY + '"' in a[i], "diff 内容异常 @" + str(i)
    assert 'translateX="' + TX + '" translateY="' + NEW_TY + '"' in b[i], "diff 内容异常 @" + str(i)
    assert 'depth="10"' in a[i], "叶子 diff 应为 d10 放置 @" + str(i)
print("全元素级 diff 断言 PASS：叶子仅 4 处 translateY 变化＋容器 sprite121 随之差异，其余零改动")
...
assert r.returncode == 0, r.stdout + r.stderr
assert_state(ET.parse(RB_XML), "回读", NEW_TY)
print("回读 FOUND：产物内四槽 translateX=-210/translateY=199/0.75 全部命中")
