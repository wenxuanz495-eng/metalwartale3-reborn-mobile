# -*- coding: utf-8 -*-
# 星际雷神 V43 修订（2026-09-18，仅 lv1；部件2 右移 0.1px——用户 directive，对齐炮口凹陷）：
# 两片（2931/2933）统一 tx 1620→1622（+2tw=右移 0.1px），ty 348/708（V42 值）不变。
# 当前态（V42）：片1 (1620,348)、片2 (1620,708)。其余零改动。
import xml.etree.ElementTree as ET
import hashlib, subprocess, sys
BS = chr(92)
sys.stdout.reconfigure(encoding='utf-8')
CUR_SWF  = r".."+BS+"swf"+BS+"sub1130.swf"
SRC_XML  = "work"+BS+"sub1130-v43-src.xml"
OUT_XML  = "work"+BS+"sub1130-v43.xml"
OUT_SWF  = "work"+BS+"sub1130-v43.swf"
RB_XML   = "work"+BS+"sub1130-v43-readback.xml"
...
assert actual == manifest_line.upper() and actual.startswith("4EA2A9EC"), actual[:8]
...
for it in sp121.find('subTags'):
    ...
    if cid in ('2931', '2933'):
        ...
        want = {'2931': ('1620', '348'), '2933': ('1620', '708')}[cid]
        assert (tx, ty) == want, f"当前态异常 {cid}: ({tx},{ty}) 应为 {want}"
        m.set('translateX', '1622')
        expect[cid] = (d, '1622', str(ty))
assert len(expect) == 2, "部件2 放置未找齐"
print("矩阵修改 PASS：2931/2933 → (1622,348)/(1622,708)（右移 0.1px）")
