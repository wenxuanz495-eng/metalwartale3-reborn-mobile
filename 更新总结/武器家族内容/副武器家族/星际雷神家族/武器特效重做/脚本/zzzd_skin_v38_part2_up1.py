# -*- coding: utf-8 -*-
# 星际雷神 V38 修订（2026-09-18，仅 lv1；部件2 上移 1px——用户 directive）：
...
assert actual == manifest_line.upper() and actual.startswith("804C1E9F"), actual[:8]
...
for it in sp121.find('subTags'):
    ...
    if cid == '2931' and d == '1':
        m = it.find('matrix')
        assert m.get('translateX') == '1640' and m.get('translateY') == '360'
        m.set('translateX', '1640')
        m.set('translateY', '340')
        expect['2931'] = ('1', '1640', '340')
    if cid == '2933' and d == '2':
        m = it.find('matrix')
        assert m.get('translateX') == '1640' and m.get('translateY') == '720'
        m.set('translateX', '1640')
        m.set('translateY', '700')
        expect['2933'] = ('2', '1640', '700')
assert len(expect) == 2, "部件2 放置未找齐"
print("矩阵修改 PASS：2931→(1640,340)、2933→(1640,700)（上移 1px）")
