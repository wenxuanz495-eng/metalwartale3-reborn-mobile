# -*- coding: utf-8 -*-
# 星际雷神 V41 修订（2026-09-18，仅 lv1；部件2 再左移 1px——用户 directive；另核实 V40 实际位移为 3px 系脚本算术笔误）：
...
assert actual == manifest_line.upper() and actual.startswith("2DA50834"), actual[:8]
...
for it in sp121.find('subTags'):
    ...
    if cid == '2931' and d == '1':
        m = it.find('matrix')
        assert m.get('translateX') == '1620' and m.get('translateY') == '344'
        m.set('translateX', '1620')
        m.set('translateY', '348')
        expect['2931'] = ('1', '1620', '348')
    if cid == '2933' and d == '2':
        m = it.find('matrix')
        assert m.get('translateX') == '1620' and m.get('translateY') == '704'
        m.set('translateX', '1620')
        m.set('translateY', '708')
        expect['2933'] = ('2', '1620', '708')
assert len(expect) == 2, "部件2 放置未找齐"
print("矩阵修改 PASS：2931→(1620,348)、2933→(1620,708)（下移 0.2px）")
