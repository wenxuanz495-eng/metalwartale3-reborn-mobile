# -*- coding: utf-8 -*-
# 星际雷神 V39 修订（2026-09-18，仅 lv1；部件2 下移 0.2px——用户 directive）：
...
assert actual == manifest_line.upper() and actual.startswith("5050B21B"), actual[:8]
...
for it in sp121.find('subTags'):
    ...
    if cid == '2931' and d == '1':
        m = it.find('matrix')
        assert m.get('translateX') == '1640' and m.get('translateY') == '340'
        m.set('translateX', '1640')
        m.set('translateY', '344')
        expect['2931'] = ('1', '1640', '344')
    if cid == '2933' and d == '2':
        m = it.find('matrix')
        assert m.get('translateX') == '1640' and m.get('translateY') == '700'
        m.set('translateX', '1640')
        m.set('translateY', '704')
        expect['2933'] = ('2', '1640', '704')
assert len(expect) == 2, "部件2 放置未找齐"
print("矩阵修改 PASS：2931→(1640,344)、2933→(1640,704)（下移 0.2px）")
