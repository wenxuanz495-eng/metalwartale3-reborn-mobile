# -*- coding: utf-8 -*-
# DoABC2 标签移植（黄金深渊五级 20260917）——沿 goldflyblade_abc_transplant.py 先例
# 10 个 mxmlc 编译存根（本体 lv1~lv5 + 子弹 lv1~lv5_bullet）插入 sub1130-gold.swf 的 SymbolClass 之前
import struct, sys, zlib, hashlib
sys.stdout.reconfigure(encoding='utf-8')

BASE = 'work/sub1130-gold.swf'
OUT = 'work/sub1130-gold-final.swf'
STUBS = ['work/stubs/stub_lv1.swf', 'work/stubs/stub_lv2.swf', 'work/stubs/stub_lv3.swf',
         'work/stubs/stub_lv4.swf', 'work/stubs/stub_lv5.swf',
         'work/stubs/stub_lv1b.swf', 'work/stubs/stub_lv2b.swf', 'work/stubs/stub_lv3b.swf',
         'work/stubs/stub_lv4b.swf', 'work/stubs/stub_lv5b.swf']

def parse_tags(body):
    tags = []
    pos = 0
    while pos < len(body):
        cl = struct.unpack('<H', body[pos:pos+2])[0]
        t = cl >> 6
        if cl & 0x3F == 0x3F:
            ln = struct.unpack('<I', body[pos+2:pos+6])[0]; hdr = 6
        else:
            ln = cl & 0x3F; hdr = 2
        if t == 0: break
        tags.append((t, body[pos+hdr:pos+hdr+ln]))
        pos += hdr + ln
    return tags

def tag_bytes(t, payload):
    if len(payload) < 0x3F:
        return struct.pack('<H', (t << 6) | len(payload)) + payload
    return struct.pack('<HI', (t << 6) | 0x3F, len(payload)) + payload

def swf_body(path):
    data = open(path, 'rb').read()
    sig = data[:3]
    if sig == b'CWS':
        return zlib.decompress(data[8:]), True
    return data[8:], False

def extract_doabc(path):
    body, _ = swf_body(path)
    nbits = body[0] >> 3
    pos = (5 + nbits*4 + 7)//8 + 4
    for t, payload in parse_tags(body[pos:]):
        if t == 82:
            return payload
    raise KeyError(path)

abc_tags = []
for s in STUBS:
    p = extract_doabc(s)
    abc_tags.append(p)
    print(f'{s}: DoABC2 {len(p)} 字节')

body, compressed = swf_body(BASE)
nbits = body[0] >> 3
rect_len = (5 + nbits*4 + 7)//8
head, rest = body[:rect_len+4], body[rect_len+4:]
tags = parse_tags(rest)
print(f'主 SWF: {len(tags)} 个标签, 压缩={compressed}')

out_tags = []
inserted = 0
for t, p in tags:
    if t == 76 and inserted == 0:
        for pabc in abc_tags:
            out_tags.append((82, pabc)); inserted += 1
    out_tags.append((t, p))
assert inserted == 10, inserted
print(f'插入 {inserted} 个 DoABC2，标签数 {len(tags)} -> {len(out_tags)}')

newbody = head + b''.join(tag_bytes(t, p) for t, p in out_tags)
sig_out = b'CWS' if compressed else b'FWS'
ver = open(BASE, 'rb').read()[3]
if compressed:
    comp = zlib.compress(bytes(newbody))
    out = sig_out + bytes([ver]) + struct.pack('<I', 8 + len(newbody)) + comp
else:
    out = sig_out + bytes([ver]) + struct.pack('<I', 8 + len(newbody)) + newbody
open(OUT, 'wb').write(out)
print('写出:', OUT, hashlib.sha256(open(OUT, 'rb').read()).hexdigest().upper()[:16])
